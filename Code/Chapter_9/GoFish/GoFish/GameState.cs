...
public class GameState
{
    ...
    public GameState(string humanPlayerName, IEnumerable<string> opponentNames,
                     Deck stock)
    {
        this.Stock = stock;
                            ...
                            Values valueToAskFor, Deck stock)
    {
        ...
        var message = $"{player} asked {playerToAsk}"
        ...
        if (cards.Count() > 0)
        {
            ...
            message += $"{playerToAsk} has {cards.Count()}"
    ...
    /// </summary>
    /// <returns>String with the winners, empty string if there are no winners</returns>
    public string CheckForWinner()
    {
        ...
        if (winners.Count() == 1) return $"The winner is {winners.First()}";
